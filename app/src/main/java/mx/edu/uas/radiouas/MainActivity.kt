...
import androidx.activity.compose.BackHandler // <--- ASEGURATE DE TENER ESTA IMPORTACIÓN
...
fun MainScreen(radioViewModel: RadioViewModel) {
    ...
    // ESTA VARIABLE CONTROLA EN QUE PANTALLA ESTÁS
    var currentSection by remember { mutableStateOf("Noticias") }

    // --- DETECCIÓN DE FULLSCREEN (Para el Video) ---
    val configuration = LocalConfiguration.current
    val isLandscape = configuration.orientation == Configuration.ORIENTATION_LANDSCAPE
    val isFullscreenMode = (currentSection == "Video" && isLandscape)

    // --- MANEJO DEL BOTÓN ATRÁS (Incidencia #3) ---
    // Si NO estamos en "Noticias", el botón Atrás nos lleva a "Noticias".
    // Si YA estamos en "Noticias", el botón funciona normal (cierra la app).
    BackHandler(enabled = currentSection != "Noticias") {
        currentSection = "Noticias"
    }

    ModalNavigationDrawer(
        drawerState = drawerState,
        // Bloqueamos el gestor del drawer si estamos en fullscreen para evitar accidentes
        gesturesEnabled = !isFullscreenMode,
        drawerContent = {
            ModalDrawerSheet {
        ...
        Scaffold(
            // 1. OCULTAMOS LA BARRA SUPERIOR SI ES FULLSCREEN
            topBar = {
                if (!isFullscreenMode) {
                    TopAppBar(
            ...
            // 2. OCULTAMOS EL MINIPLAYER SI ES FULLSCREEN
            bottomBar = {
                if (!isFullscreenMode) {
            ...
            // 3. ELIMINAMOS EL RELLENO (PADDING) SI ES FULLSCREEN
            val contentPadding = if (isFullscreenMode) PaddingValues(0.dp) else paddingValues
